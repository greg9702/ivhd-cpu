...
void parseArg(int argc, char *argv[]) {
    if (argc != 8) {
        ...
        cerr << "./ivhd dataset_file knn_file iterations random_neighbours "
                "experiment_name "
                "algorithm_name seed\n";
        exit(-1);
    }
...
Caster *getCaster(int n, function<void(float)> onError,
    ...
    if (algorithm_name == "adadelta_sync") {
        return new CasterNesterov(n, onError, onPos);
    } else {
        cerr << "Invalid algorithm_name. Expected one of: 'adadelta_sync' ";
        exit(-1);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    auto onPos = [&](vector<std::pair<float, float>> &positions) -> void {
        now = system_clock::now();
        auto time = time_point_cast<milliseconds>(now).time_since_epoch().count() -
                    start - offset;
    ...
    for (int i = 0; i < n; i++) {
        caster.positions[i].first = rand() % 100000 / 100000.0;
        caster.positions[i].second = rand() % 100000 / 100000.0;
    }
    ...
    for (int i = 0; i < n; i++) {
        results << caster.positions[i].first << " " << caster.positions[i].second << " "
                << data.labels[i] << endl;
    }
